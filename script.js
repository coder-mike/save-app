...
function updateState() {
  ...
  if (window.state.actions)
    window.state = rebuildState(window.state.id, window.state.actions);
...
function makeEditable(el, { obj, field, readTransform, writeTransform, read, write, requiresRender }) {
  requiresRender ??= true;
  writeTransform ??= v => v;
  readTransform ??= v => v;
  read ??= () => readTransform(obj[field]);
  write ??= value => obj[field] = writeTransform(value);
...
function foldAction(state, action) {
  ...
  switch (action.type) {
    ...
    case 'ListSetBudget': {
      const list = findList(action.listId);
      if (list) list.budget = action.budget;
      break;
    }
...
function rebuildState(id, actions) {

  return actions.reduce(foldAction, { ...emptyState(), id });
}
